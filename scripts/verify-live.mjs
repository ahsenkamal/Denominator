...
const input = {
  ...
  claim: `This pool's USD volume increased by 50% on ${day(1)} compared with ${day(2)}.`,
};
const headers = {
  "Content-Type": "application/json",
  ...(process.env.DEMO_ACCESS_CODE
    ? { "x-demo-access-code": process.env.DEMO_ACCESS_CODE }
    : {}),
};
...
console.log(
  "PASS: live Gemini interpretation preserves metric, percentage and UTC dates.",
);
const receipt = await post("/api/verify", {
  ...input,
...
await writeFile(
  "/tmp/denominator-live-receipt.json",
  JSON.stringify(receipt, null, 2),
);
console.log(
  JSON.stringify({
    result: "PASS: live Graph evidence and receipt integrity",
    verdict: receipt.evaluation.verdict,
    baseline: receipt.evaluation.baseline,
    current: receipt.evaluation.current,
    percentChange: receipt.evaluation.percentChange,
    block: receipt.snapshot.provenance.block.number,
