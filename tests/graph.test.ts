...
const plan: ClaimPlan = {
  ...
  interpretation: "Five times the baseline.",
};
const hash = `0x${"a".repeat(64)}`;
const metadata = {
  deployment: "QmExample",
  hasIndexingErrors: false,
  block: { number: 123, hash, timestamp: dateTimestamp("2026-09-13") },
};
function data() {
  return {
    _meta: { ...metadata, block: { number: 123, hash: null, timestamp: null } },
    pool: {
      id: POOLS[0].id,
      feeTier: "500",
    ...
    poolDayDatas: [
      { date: dateTimestamp("2026-09-12"), volumeUSD: "1000", txCount: "100" },
    ],
  };
}
function setup(second = data()) {
  ...
  const fetch = vi
    .fn()
    .mockResolvedValueOnce(Response.json({ data: { _meta: metadata } }))
...
describe("Graph provenance", () => {
  it("pins by hash and retains the matching preflight header when historical fields are null", async () => {
    const fetch = setup();
    const snapshot = await fetchSnapshot(POOLS[0].id, plan);
  ...
  it("rejects an unexpected deployment", async () => {
    const response = data();
    response._meta.deployment = "QmDifferent";
    setup(response);
    await expect(fetchSnapshot(POOLS[0].id, plan)).rejects.toMatchObject({
      code: "SOURCE_CHANGED",
    });
  });
  it("rejects a mismatched block number", async () => {
    const response = data();
    response._meta.block.number = 124;
    setup(response);
    await expect(fetchSnapshot(POOLS[0].id, plan)).rejects.toMatchObject({
      code: "SOURCE_CHANGED",
    });
  ...
  it("does not execute arbitrary pool addresses", async () => {
    const fetch = setup();
    await expect(
      fetchSnapshot(`0x${"1".repeat(40)}`, plan),
    ).rejects.toMatchObject({ code: "POOL_UNSUPPORTED" });
