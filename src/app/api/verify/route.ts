...
export async function POST(request: Request) {
  try {
    ...
    try { validateWindow(input.plan); } catch (error) { throw new AppError("DATE_WINDOW", (error as Error).message); }
    ...
    const payload = { version: "1.0" as const, claim: input.claim, poolLabel: `${pool.label} · ${pool.fee}`, plan: input.plan, evaluation, snapshot, interpretationConfirmedByUser: true as const };
